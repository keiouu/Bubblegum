...
<li class="active">{% i18n "admin_update" %}</li>
{% endblock %}

{% block body %}
<div class="hero-unit">
	<h2>{% i18n "admin_update" %}</h2>
	<?php
	...
	print '<br />' . $request->i18n['admin_check_framework'];
	if (TPUpdater::needs_update("framework")) {
		$version = TPUpdater::get_remote_version("framework");
		print '  ' . str_replace("#version#", $version, $request->i18n['admin_check_version']);
		print '<div class="update_entry">' . $request->i18n['admin_update_framework'];
	...
	foreach ($apps as $app) {
		...
		print str_replace("#app#", $app_name, $request->i18n['admin_check_app']);
		if (TPUpdater::needs_update($app_name)) {
			$version = TPUpdater::get_remote_version($app_name);
			print '  ' . str_replace("#version#", $version, $request->i18n['admin_check_version']);
			print '<div class="update_entry">' . str_replace("#app#", $app_name, $request->i18n['admin_update_app']);
